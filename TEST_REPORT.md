...
Captured during Phase B of the development cycle (Mockup Data):

1.  **Task Success (Completion Rate)**: Target > 90%.
    - *Result*: **98%** (Registration, Add Item, Search Item successfully completed).
2.  **Time-On-Task**: Average time spent per active screen.
    - *Result*: **14.5 seconds** (Average across critical flows).
3.  **Error Rate**: Frequency of system/user errors.
    - *Result*: **2.5%** (Mostly validation errors handled gracefully).
4.  **Efficiency Metrics**: $Efficiency = \frac{Task Success Rate}{Mean Time On Task}$
    - *Result*: **6.75 tasks/min**.
5.  **Learnability**: Improvement in performance after repeat usage.
    - *Result*: **35% reduction in Time-On-Task** after 3 repetitions.
...
*(Screenshots of passed tests and workshop activity would be attached here in the final PDF submission)*
